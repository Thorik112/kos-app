...
public class p_pembayaran_pemilik extends javax.swing.JFrame {
    
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(p_pembayaran_pemilik.class.getName());

    private JPanel content;
 
    // ===================================================================
    // DESIGN TOKENS  (same as p_dashboard_pemilik)
    // ===================================================================
    ...
    public p_pembayaran_pemilik() {
        setTitle("KosApp \u2014 Pembayaran Masuk");
    ...
    // ===================================================================
    // SIDEBAR  (identical helper pattern to p_dashboard_pemilik)
    // ===================================================================
    private JPanel buildSidebar() {
        ...
        JButton btnBayar  = sidebarBtn("Pembayaran",  "\u25CE", true);   // aktif
        ...
        btnBayar.addActionListener(e  -> { /* sudah di halaman ini */ });
    ...
    private JPanel buildSectionHeader() {
        ...
        JLabel t = new JLabel("Pembayaran Masuk");
        t.setFont(FONT_H1);
        t.setForeground(C_TEXT_PRIM);
        t.setAlignmentX(LEFT_ALIGNMENT);
 
        JLabel s = new JLabel("Kelola dan verifikasi pembayaran dari penyewa");
        s.setFont(FONT_BODY);
    ...
    // PAYMENT CARD
    // ===================================================================
    private JPanel createCard(int id, String user, double total, String bukti, String status) {
 
        // Determine accent color by status
        Color barColor;
        Color badgeBg;
        Color badgeFg;
        String badgeText;
        ...
        switch (status.toLowerCase()) {
            ...
            default:
                barColor = C_ORANGE;
                badgeBg  = C_ORANGE_BG;
                badgeFg  = C_ORANGE_TEXT;
                badgeText = "PENDING";
                break;
        }
 
        // Outer wrapper with bottom margin
        JPanel wrapper = new JPanel(new BorderLayout());
        ...
        JPanel card = new JPanel(new BorderLayout(14, 0)) {
            ...
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.fillRoundRect(0, 18, 5, h - 36, 3, 3);
                g2.dispose();
            }
            @Override public boolean isOpaque() { return false; }
        };
        ...
        thumb.setPreferredSize(new Dimension(84, 84));
        thumb.setLayout(new BorderLayout());
 
        try {
            String path = "E:\\NetBeansProjects\\Kos_TA\\src\\images\\" + bukti;
            ImageIcon icon = new ImageIcon(path);
            Image img = icon.getImage().getScaledInstance(84, 84, Image.SCALE_SMOOTH);
            JLabel imgLbl = new JLabel(new ImageIcon(img));
            imgLbl.setBorder(new EmptyBorder(0, 0, 0, 0));
            thumb.add(imgLbl);
        } catch (Exception ex) {
            JLabel noImg = new JLabel("No Image", SwingConstants.CENTER);
            noImg.setFont(FONT_SMALL);
            noImg.setForeground(C_TEXT_SEC);
            thumb.add(noImg);
        }
        ...
        JLabel nameLbl = new JLabel(user != null ? user : "-");
        nameLbl.setFont(new Font("Segoe UI", Font.BOLD, 14));
        nameLbl.setForeground(C_TEXT_PRIM);
        nameLbl.setAlignmentX(LEFT_ALIGNMENT);
 
        JLabel totalLbl = new JLabel("Total: Rp " + formatHarga(String.valueOf((long) total)));
        totalLbl.setFont(FONT_BODY);
        totalLbl.setForeground(C_TEXT_SEC);
        ...
        final Color finalBadgeFg = badgeFg;
        ...
        badge.setForeground(finalBadgeFg);
        badge.setBorder(new EmptyBorder(5, 14, 5, 14));
        badge.setAlignmentX(RIGHT_ALIGNMENT);
 
        // Action buttons
        JButton btnLihat = ghostButton("Lihat");
        JButton btnAcc   = darkButton("ACC");
        JButton btnTolak = dangerButton("Tolak");
 
        boolean isPending = status.equalsIgnoreCase("pending");
        btnAcc.setEnabled(isPending);
        btnTolak.setEnabled(isPending);
        if (!isPending) {
            btnAcc.setForeground(C_TEXT_SEC);
            btnTolak.setForeground(C_TEXT_SEC);
            btnAcc.setCursor(Cursor.getDefaultCursor());
            btnTolak.setCursor(Cursor.getDefaultCursor());
        }
 
        btnLihat.addActionListener(e ->
            new lihat_gambar("E:\\NetBeansProjects\\Kos_TA\\src\\images\\" + bukti).setVisible(true));
        btnAcc.addActionListener(e   -> updateStatus(id, "lunas"));
        btnTolak.addActionListener(e -> updateStatus(id, "gagal"));
    ...
    private void loadData() {
        ...
        try {
            ...
            String sql =
                ...
                "LEFT JOIN kos ks    ON k.id_kos     = ks.id_kos " +
                "WHERE ks.id_user = ? " +
            ...
            if (!hasData) {
                JLabel empty = new JLabel("Belum ada pembayaran masuk.");
                empty.setFont(FONT_BODY);
                empty.setForeground(C_TEXT_SEC);
    ...
    private void updateStatus(int idPembayaran, String status) {
        try {
            Connection conn = koneksi.connect();
            PreparedStatement pst = conn.prepareStatement(
                "UPDATE pembayaran SET status_pembayaran=? WHERE id_pembayaran=?");
            pst.setString(1, status);
            pst.setInt(2, idPembayaran);
            pst.executeUpdate();
    ...
    private void startAutoRefresh() {
        new javax.swing.Timer(5000, e -> loadData()).start();
    }
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new p_pembayaran_pemilik().setVisible(true));
    }
